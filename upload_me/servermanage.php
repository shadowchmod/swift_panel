...
case "restart" :
case "stop" :
    $rows = ( "SELECT `serverid`, `clientid`, `boxid`, `name`, `user`, `password`, `online`, `status` FROM `server` WHERE `serverid` = '".$serverid."' && `clientid` = '".$_SESSION['clientid']."' LIMIT 1" );
    if ( $rows['online'] == "Stopped" || $rows['status'] == "Suspended" )
    {
    ...
    if ( $task == "stop" )
    {
        $message = "Server Stopped: <a href=\"serversummary.php?id=".$serverid."\">".$rows['name']."</a> (Client)";
        ( "INSERT INTO `log` SET `clientid` = '".$rows['clientid']."', `serverid` = '".$serverid."', `boxid` = '".$rows['boxid']."', `message` = '".$message."', `name` = '".$_SESSION['clientfirstname']." ".$_SESSION['clientlastname']."', `ip` = '".$_SERVER['REMOTE_ADDR']."'" );
...
case "start" :
    ...
    if ( $rows['online'] == "Started" || $rows['status'] == "Suspended" )
    {
    ...
    if ( $task == "start" )
    {
        $message = "Server Started: <a href=\"serversummary.php?id=".$serverid."\">".$rows['name']."</a> (Client)";
        ( "INSERT INTO `log` SET `clientid` = '".$rows['clientid']."', `serverid` = '".$serverid."', `boxid` = '".$rows['boxid']."', `message` = '".$message."', `name` = '".$_SESSION['clientfirstname']." ".$_SESSION['clientlastname']."', `ip` = '".$_SERVER['REMOTE_ADDR']."'" );
    ...
    {
        $message = "Server Restarted: <a href=\"serversummary.php?id=".$serverid."\">".$rows['name']."</a> (Client)";
        ( "INSERT INTO `log` SET `clientid` = '".$rows['clientid']."', `serverid` = '".$serverid."', `boxid` = '".$rows['boxid']."', `message` = '".$message."', `name` = '".$_SESSION['clientfirstname']." ".$_SESSION['clientlastname']."', `ip` = '".$_SERVER['REMOTE_ADDR']."'" );
